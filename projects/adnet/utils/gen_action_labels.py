...
def gen_action_pos_neg_labels(num_actions, opts, bb_samples, gt_bbox):
    num_samples = 1

    action_labels_pos = np.zeros([num_actions, num_samples])
    ...
    for j in range(len(bb_samples)):
        ...
        action = np.zeros(num_actions)
        action[max_action] = 1
        action_labels_pos[:, j] = action
        action_neg = np.zeros(num_actions)
        action_neg[min_action] = 1
        action_labels_neg[:, j] = action_neg

        # return bbox back
        # bbox[0] = bbox[0] - 0.5 * bbox[2]
        # bbox[1] = bbox[1] - 0.5 * bbox[3]

    return action_labels_pos,action_labels_neg  # in real matlab code, they also return overs
