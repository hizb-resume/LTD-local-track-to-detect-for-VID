            ...
            for t_id,curr_bbox in enumerate(frame_pred['bbox']):
                ...
                if curr_score < 0.5:
                    ...
                    if euclidean_distance.item()<0.5:
                        filename1="temimg/v%d-f%d-t%d-siam%d-pre.JPEG"%(vidx,frame_idx,t,euclidean_distance.item())
                        # cv2.imwrite(filename1, x0.numpy())
                        cv2.imwrite(filename1, x0_crop)
                        filename2 = "temimg/v%d-f%d-t%d-siam%d-cur.JPEG"%(vidx,frame_idx,t,euclidean_distance.item())
