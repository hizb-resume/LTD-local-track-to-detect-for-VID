...
class RolloutStorage(object):
    ...
    def feed_forward_generator(self,
        ...
        for indices in sampler:
            obs_batch = self.obs[:self.step].view(-1, *self.obs.size()[2:])[indices]
            recurrent_hidden_states_batch = self.recurrent_hidden_states[:self.step].view(
                -1, self.recurrent_hidden_states.size(-1))[indices]
            actions_batch = self.actions[:self.step].view(-1,
                                              self.actions.size(-1))[indices]
            value_preds_batch = self.value_preds[:self.step].view(-1, 1)[indices]
            return_batch = self.returns[:self.step].view(-1, 1)[indices]
            masks_batch = self.masks[:self.step].view(-1, 1)[indices]
            old_action_log_probs_batch = self.action_log_probs[:self.step].view(-1,
                                                                    1)[indices]
